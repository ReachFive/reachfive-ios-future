...
{company} SDKs are published on link:https://www.swift.org/documentation/package-manager/[Swift Package Manager^] link:https://cocoapods.org/pods/IdentitySdkCore[Cocoapods^] as independent modules. 
The latest version is `{sdk-ios-latest-version}`.
...
The core SDK contains all the main tools and interfaces, and methods related to standard authentication by identifier and password. 
...
----
+
NOTE: If you're using the latest version of the Facebook API, please remove the `user_gender` scope from the {company} client configuration to prevent any issues.
...
+
TIP: See <<Info.plist reference>> for a comprehensive look a the file.
...
----
<1> The URL Scheme is the reversed Google Client ID (not {company}'s xref:docs:ROOT:clients.adoc[clientID]), which is your Google Client ID with the order of the dot-delimited fields reversed. For example, `com.googleusercontent.apps.abcdefg-1234567890`.
+
TIP: See <<Info.plist reference>> for a comprehensive look a the file.
